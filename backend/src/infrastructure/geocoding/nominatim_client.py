...
from src.domain.models.location import GeoCoordinates, Location
...
class NominatimClient:
    ...
    async def geocode(self, query: str) -> Location | None:
        ...
        try:
            ...
            logger.exception("nominatim_geocode_error", query=query, error=str(e))
            return None

    async def reverse_geocode(self, lat: float, lng: float) -> Location | None:
        ...
        try:
            ...
            logger.exception("nominatim_reverse_error", lat=lat, lng=lng, error=str(e))
            return None
