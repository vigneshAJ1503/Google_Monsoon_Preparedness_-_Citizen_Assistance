    ...
    async def fetch_official_alerts(self) -> list[Alert]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    "sachet_feed_http_error", status_code=response.status_code,
                )
                return []
            ...
            for entry in feed.entries:
                ...
                if (
                    ...
                    or "extreme" in title_lower
                    or "red alert" in title_lower
                ):
                    severity = AlertSeverity.SEVERE
                elif (
                    "heavy rain" in title_lower
                    or "warning" in title_lower
                    or "orange alert" in title_lower
                ):
            ...
            logger.exception("ndma_feed_parsing_failed_degrading_safely", error=str(e))
            return []
