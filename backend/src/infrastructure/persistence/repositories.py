...
from src.domain.models.checklist import ChecklistItem, ChecklistStatus, ChecklistCategory
...
class HouseholdRepository:
    ...
    async def get_by_id(self, id: UUID) -> Optional[HouseholdProfile]:
...
class ChecklistRepository:
    ...
    async def get_by_household(self, household_id: UUID) -> List[ChecklistItem]:
        ...
        stmt = select(ChecklistItemModel).where(ChecklistItemModel.household_id == household_id).order_index = ChecklistItemModel.priority.desc()
        # Fix ordering
        stmt = select(ChecklistItemModel).where(ChecklistItemModel.household_id == household_id).order_by(ChecklistItemModel.priority.desc(), ChecklistItemModel.created_at.asc())
        result = await self.db.execute(stmt)
    ...
    async def save_all(self, items: List[ChecklistItem]) -> List[ChecklistItem]:
        ...
        for item in items:
            if item.id:
                stmt = select(ChecklistItemModel).where(ChecklistItemModel.id == UUID(item.id))
                result = await self.db.execute(stmt)
                model = result.scalar_one_or_none()
            else:
                model = None
    ...
    async def delete_by_household_and_category(self, household_id: UUID, category: ChecklistCategory):
...
class AlertRepository:
    ...
    async def get_active_alerts(self) -> List[Alert]:
        ...
        stmt = select(AlertModel).where(
            ...
            (AlertModel.expires_at == None) | (AlertModel.expires_at > now)
        ).order_by(AlertModel.triggered_at.desc())
        result = await self.db.execute(stmt)
        models = result.scalars().all()
        return [self._to_domain(m) for m in models]

    async def get_by_rule_and_location(self, rule_id: str, lat: float, lng: float, min_time: datetime) -> List[Alert]:
        ...
        stmt = select(AlertModel).where(
            ...
            AlertModel.triggered_at >= min_time
        )
        result = await self.db.execute(stmt)
        models = result.scalars().all()
        ...
        for m in models:
            if m.location_lat is not None and m.location_lng is not None:
                if round(m.location_lat, 2) == lat_rounded and round(m.location_lng, 2) == lng_rounded:
...
class PreparednessPlanRepository:
    ...
    async def get_by_household(self, household_id: UUID) -> Optional[PreparednessPlanModel]:
        """Fetch plan by household ID."""
        stmt = select(PreparednessPlanModel).where(
            PreparednessPlanModel.household_id == household_id
        ).order_by(PreparednessPlanModel.generated_at.desc())
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()

    async def save(self, household_id: UUID, plan_data: dict, weather_context: dict, risk_level: str, ttl_hours: int = 1):
        """Save a generated preparedness plan."""
        # Expire older plans
        stmt = delete(PreparednessPlanModel).where(PreparednessPlanModel.household_id == household_id)
        await self.db.execute(stmt)
