...
async def _translate_plan_actions(
    plan: PreparednessePlan, lang: str,
) -> PreparednessePlan:
    """Translate all actionable text in a plan to target language."""
    ...
    for item in plan.emergency_kit:
        translated_kit.append(await translation_service.translate_text(item, lang))
...
async def generate_preparedness_plan(
    ...
    if lang != "en":
        ...
        plan = await _translate_plan_actions(plan, lang)
