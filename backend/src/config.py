...
class Settings(BaseSettings):
    ...
    gemini_max_output_tokens: int = Field(
        default=2048, alias="GEMINI_MAX_OUTPUT_TOKENS",
    )
    ...
    weather_cache_ttl_seconds: int = Field(
        default=300, alias="WEATHER_CACHE_TTL_SECONDS",
    )
    forecast_cache_ttl_seconds: int = Field(
        default=1800, alias="FORECAST_CACHE_TTL_SECONDS",
    )
    ...
    alert_data_max_age_seconds: int = Field(
        default=600, alias="ALERT_DATA_MAX_AGE_SECONDS",
    )
