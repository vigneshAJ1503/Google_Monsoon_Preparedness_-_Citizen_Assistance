...
class TravelService:
    ...
    async def get_travel_advisory(
        ...
        # 2. Fetch Weather
        origin_weather = await weather_service.get_weather_context(
            origin_lat, origin_lng,
        )
        dest_weather = await weather_service.get_weather_context(dest_lat, dest_lng)

        # 3. Fetch Alerts
        origin_alerts = await alert_service.get_alerts_for_location(
            origin_lat, origin_lng, db,
        )
        dest_alerts = await alert_service.get_alerts_for_location(
            dest_lat, dest_lng, db,
        )

        combined_alerts = list(
            {a.id: a for a in (origin_alerts + dest_alerts) if a.id}.values(),
        )
        ...
        if dest_rain >= 100.0 or any(
            a.severity.value == "SEVERE" for a in combined_alerts
        ):
            risk_level = TravelRiskLevel.AVOID
            reasons.append(
                "Extreme rainfall forecast or active severe alert along the route/destination.",
            )
        elif dest_rain >= 50.0 or any(
            a.severity.value == "HIGH" for a in combined_alerts
        ):
            risk_level = TravelRiskLevel.HIGH
            reasons.append(
                "Heavy rainfall forecast or active warnings. Waterlogging of roads is highly likely.",
            )
        elif dest_rain >= 25.0:
            risk_level = TravelRiskLevel.MODERATE
            reasons.append(
                "Moderate rainfall expected. Watch out for slow traffic and minor water pooling.",
            )

        if dest_wind >= 60.0:
            risk_level = max(risk_level, TravelRiskLevel.HIGH)
            reasons.append(
                "High wind warnings. Risk of fallen tree branches or structural blockages.",
            )

        # Default reason
        if risk_level == TravelRiskLevel.LOW:
            reasons.append(
                "No active alerts or severe weather conditions reported along the route.",
            )
        ...
        if llm_ready:
            try:
                alert_text = (
                    "; ".join(
                        f"[{a.severity.value}] {a.title}" for a in combined_alerts
                    )
                    or "None"
                )
                ...
                logger.exception("travel_advisory_llm_failed", error=str(e))
