...
BASE_CHECKLIST_TEMPLATES = [
    # Essentials
    {
        "title": "Charge all mobile phones and power banks to full capacity.",
        "category": ChecklistCategory.ESSENTIALS,
        "priority": 10,
    },
    {
        "title": "Keep emergency flashlights/torches and spare batteries ready.",
        "category": ChecklistCategory.ESSENTIALS,
        "priority": 9,
    },
    {
        "title": "Keep a physical copy of important emergency contact numbers.",
        "category": ChecklistCategory.COMMUNICATION,
        "priority": 8,
    },
    # Food & Water
    {
        "title": "Store clean drinking water (at least 4 liters per person per day for 3 days).",
        "category": ChecklistCategory.FOOD_WATER,
        "priority": 9,
    },
    {
        "title": "Stock up on dry, ready-to-eat foodstuffs (biscuits, parched rice).",
        "category": ChecklistCategory.FOOD_WATER,
        "priority": 8,
    },
    # Medical
    {
        "title": "Check and replenish first-aid kit supplies and personal prescription medicines.",
        "category": ChecklistCategory.MEDICAL,
        "priority": 9,
    },
    {
        "title": "Keep ORS (Oral Rehydration Salts) packets and chlorine tablets on hand.",
        "category": ChecklistCategory.MEDICAL,
        "priority": 7,
    },
    # Documents
    {
        "title": "Put identification cards, insurance cards, and deeds in a zip-lock waterproof pouch.",
        "category": ChecklistCategory.DOCUMENTS,
        "priority": 8,
    },
    # Property
    {
        "title": "Check gutters and clean local courtyard drain channels.",
...
class ChecklistService:
    ...
    async def generate_initial_items(
        ...
        for t in WEATHER_CHECKLIST_TEMPLATES:
            ...
            if (
                "min_forecast_rain_mm" in t
                and forecast_rain >= t["min_forecast_rain_mm"]
            ):
                triggered = True
    ...
    async def update_item_status(
        ...
        logger.info(
            "checklist_item_status_updated", item_id=item_id, status=status.value,
        )
        return saved[0]
